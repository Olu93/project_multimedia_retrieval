...
if __name__ == "__main__":
    ...
    weights_best_all_features = ([3.84]) + ([252.79] * count_hists) + ([2.89] * count_skeletons)
    function_pipeline_cowaeu = [cosine] + ([wasserstein_distance] * count_hists) + ([cityblock] * count_skeletons)

    k_all_db_results_cowaeu = evaluator.perform_matching_calculate_metrics(function_pipeline_cowaeu, weights_best_all_features, k_full_db_switch=True)
    k_all_db_results_cowaeu.to_json("trash/k_all_db_results_cowaci_bw.json")
    k_all_db_results_cowaeu = pd.read_json("trash/k_all_db_results_cowaci_bw.json")

    k_all_db_results_cowaeu = evaluator.metric_F1(k_all_db_results_cowaeu, evaluator.mesh_classes_count, k=10, use_class_k=False, weightbool=True, weight=0.25)
