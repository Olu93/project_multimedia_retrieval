...
class Evaluator:
    ...
    def perform_matching_calculate_mean_average_metrics(self, function_pipeline, weights):
        match_results = pd.DataFrame(
            columns=['id', 'class', 'k', 'matches', 'matches_class', 'precision', 'recall', 'F1score'])
        ...
        for index, row in enumerate(self.feature_db_raw):
            if row['label'] in top_3_classes.keys():
                for k in np.linspace((top_3_classes.get(row['label']) - top_3_classes.get(row['label']) / 3),
                    ...
                    ids, distance_values = self.query_matcher.match_with_db(row,
                                                                            k=int(k),
                    ...
                    match_results = match_results.append(
                        {'id': index, 'class': row['label'], 'k': int(k), 'matches': ids,
    ...
    def perform_matching_calculate_metrics(self, function_pipeline, weights, k_full_db_switch=False):
        match_results = pd.DataFrame(
            columns=['id', 'class', 'matches', 'matches_class', 'distances', 'precision', 'recall', 'F1score'])
        for index, row in enumerate(self.feature_db_raw):
            ...
            if class_label in self.mesh_classes_count.keys():
                if k_full_db_switch:
                    k = np.sum(self.mesh_classes_count.values)
                else:
                    k = (int(self.mesh_classes_count.get(class_label)))
            else:
                continue
            row.pop("timestamp", None)
            ids, distance_values, clabels = self.query_matcher.match_with_db(row,
                                                                    k=k,
                                                                    distance_functions=function_pipeline,
                                                                    weights=weights)
            # Remove the queried mesh from results
            ids.remove(row['name'])
            distance_values = np.delete(distance_values, 0)
            clabels.pop(0)
            # matches_class = [self.features_df_raw[self.features_df_raw.index == id]['label'][0] for id in ids]
            # Calculate metrics
            # Correct matched meshes
            ...
            if (precision + recall) == 0:
                ...
                F1score = 2 * ((precision * recall) / (precision + recall))
            match_results = match_results.append(
                {'id': index, 'class': class_label, 'matches': ids, 'matches_class': clabels, 'distances': distance_values.tolist(),
                 'precision': precision, 'recall': recall, 'F1score': F1score}, ignore_index=True)
            print(index, " of ", len(self.feature_db_raw))
    ...
    def plot_metric_class_aggregate(self, F1_means_list, used_func_list):
        ...
        if isinstance(F1_means_list[0], list):
            for i in range(len(F1_means_list)):
                ...
                ax.bar(np.arange(0, (len(self.mesh_classes_count.keys()))), F1_means_list[i], bottom=bottom,
                       color=bar_colors[i], width=barw,
                       align='center')
            plt.xticks(np.arange(0, (len(self.mesh_classes_count.keys()))), self.mesh_classes_count.keys())
        else:
            ax.bar(np.arange(0, (len(F1_means_list[0]))), F1_means_list[0]['F1score'].to_list(), bottom=bottom,
                   color='b', width=barw,
                   align='center')
    ...
    def calc_weighted_metric(self, results, metric):
        weighted_class_metric_means = {classid: (np.mean(results[results['class'] == classid][
                                                   metric]) * self.mesh_classes_count.get(classid)) / np.sum(self.mesh_classes_count.values) for classid in self.mesh_classes_count.keys()}
        weighted_class_metric_means_df = pd.DataFrame({"class": weighted_class_metric_means.keys(), "bmean": weighted_class_metric_means.values()})
...
if __name__ == "__main__":
    ...
    function_pipeline = [cosine] + ([cityblock] * n_distributionals)
    ...
    cmr_F1_means = [np.mean(cosine_manhatten_results[cosine_manhatten_results['class'] == classid]['F1score']) for
                    classid in evaluator.mesh_classes_count.keys()]
    ...
    cwr_F1_means = [np.mean(cosine_wasserstein_results[cosine_wasserstein_results['class'] == classid]['F1score']) for
                    classid in evaluator.mesh_classes_count.keys()]
    ...
    ccr_F1_means = [np.mean(cosine_cosine_results[cosine_cosine_results['class'] == classid]['F1score']) for
                    classid in evaluator.mesh_classes_count.keys()]
