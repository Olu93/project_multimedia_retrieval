...
class Evaluator:
    ...
    def plot_metric_class_aggregate(self, classes_and_metrics, metric, descr):
        fig = plt.figure(figsize=(20, 5))
        pal = sns.color_palette("Greens_d", len(classes_and_metrics))
        rank = classes_and_metrics.sort_values(by='F1_cociwa', ascending=True)['F1_cociwa'].argsort().argsort()
        ax = sns.barplot(data=classes_and_metrics.sort_values(by='F1_cociwa', ascending=True), x='class', y='F1_cociwa', palette=np.array(pal)[rank])
    ...
    def generate_class_dists_heatmap(k_all_results, slice = False, descr = "heatmap1", div_classes_len=4):
        ...
        if not slice:
            ...
            ax = sns.heatmap(pv_table, xticklabels=True, yticklabels=True, cbar=False)
            ax.xaxis.tick_top()
            ax.set_xlabel(" ")
            ax.set_ylabel(" ")
...
if __name__ == "__main__":
    ...
    weights = ([1]) + ([1] * n_distributionals) + ([0] * count_skeletons)
    # weights = ([1]) + ([1] * count_hists) + ([0] * count_skeletons)
    ...
    # Cosine + Manhaten + EMD
    # k_all_db_results_cociwa = evaluator.perform_matching_calculate_metrics(function_pipeline_cociwa, weights, k_full_db_switch=True)
    ...
    evaluator.plot_metric_class_aggregate(class_metric_means[["class", "F1_cociwa"]], "F1score", "Scalars: Cosine, Distr: Manhatten")

    # Cosine + EMD + EMD
    # k_all_db_results_cowawa = evaluator.perform_matching_calculate_metrics(function_pipeline_cowawa, weights, k_full_db_switch=True)
    ...
    # evaluator.calc_weighted_metric(k_all_db_results_cowawa, "F1score", "F1")
