...
class Evaluator:
    ...
    def perform_matching_calculate_metrics(self, query_function):
        match_results = pd.DataFrame(columns=['id', 'class', 'matches', 'matches_class', 'precision', 'recall', 'F1score'])
        for index, row in self.features_df.iterrows():
            ...
            if class_label in self.mesh_classes_count.keys():
                ...
                continue
            ids, distance_values = self.query_matcher.compare_features_with_database(row.drop(columns="label"), k=class_k, distance_function=query_function)
            ...
            match_results = match_results.append({'id': index, 'class': class_label, 'matches': ids, 'matches_class': matches_class, 'precision': precision, 'recall': recall, 'F1score': F1score}, ignore_index=True)

        return match_results

    def plot_metric_class_aggregate(self, results_df, str_distance_function):
            F1means = [np.mean(results_df[results_df['class'] == classid ]['F1score']) for classid in self.mesh_classes_count.keys()]
...
if __name__ == "__main__":
    evaluator = Evaluator()
    cosine_results = evaluator.perform_matching_calculate_metrics(QueryMatcher.cosine_distance)
